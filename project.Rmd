...
```{r}
vars <- row.names(imp[order(imp[,"MeanDecreaseAccuracy"], decreasing=TRUE),])
vars[1:10]
```
...
## Cross Validation for Top x (1 <= x <= 20) Predictor Variables
...
```{r}
set.seed(123)
smp_size <- floor(0.2 * nrow(UFC.new))
train_ind <- sample(seq_len(nrow(UFC.new)), size = smp_size)
train <- UFC.new[train_ind, ]
test <- UFC.new[-train_ind, ]
```
...
svm_cross_val <- function(vars, min_preds, max_preds)
{
  ...
  {
    predictors <- paste(vars[1:i], collapse=" + ")
    svmfit <- svm(as.formula(paste("win", predictors, sep=" ~ ")), data = train, kernel = "linear",
                  cost = 10, scale=TRUE)
    ypred <- predict(svmfit, test)
    accuracy <- length(ypred[test$win==ypred]) / length(ypred)
    print(sprintf("predictors: %s", predictors))
...
```{r}
svm_cross_val(vars, 1, 40)
```


```{r}
svm_radial_cross_val <- function(vars, min_preds, max_preds)
{
  for(i in min_preds:max_preds)
  {
    predictors <- paste(vars[1:i], collapse=" + ")
    svmfit <- svm(as.formula(paste("win", predictors, sep=" ~ ")), data = train, kernel = "radial", gamma=1,
                  cost = 10, scale=TRUE)
    ypred <- predict(svmfit, test)
    accuracy <- length(ypred[test$win==ypred]) / length(ypred)
    print(sprintf("predictors: %s", predictors))
...
```{r}
svm_radial_cross_val(vars, 1, 40)
```

```{r}
svm_poly_cross_val <- function(vars, min_preds, max_preds)
{
  for(i in min_preds:max_preds)
  {
    predictors <- paste(vars[1:i], collapse=" + ")
    svmfit <- svm(as.formula(paste("win", predictors, sep=" ~ ")), data = train, kernel = "polynomial", degree = 3, gamma=1, coef0 = 0,
                  cost = 10, scale=TRUE)
    ypred <- predict(svmfit, test)
    accuracy <- length(ypred[test$win==ypred]) / length(ypred)
    print(sprintf("predictors: %s", predictors))
...
```{r}
predictors <- paste(vars[1:1], collapse=" + ")
treefit <- tree(as.formula(paste("win", predictors, sep=" ~ ")), data = train)
pred <- predict(treefit, test)[,2]
ypred <- ifelse(pred > 0.5, 1, 0)
accuracy <- length(ypred[test$win==ypred]) / length(ypred)
print(sprintf("predictors: %s", predictors))
print(sprintf("Test prediction accuracy: %f", accuracy))
...
```{r}
tree_cross_val <- function(vars, min_preds, max_preds)
{
  for(i in min_preds:max_preds)
  {
    predictors <- paste(vars[1:i], collapse=" + ")
...
```{r}
lmfit <- glm(win ~ age_dif + reach_dif + avg_opp_CTRL_time.seconds._dif + weightclass + avg_TD_att_dif + height_dif + avg_SIG_STR_pct_dif + weightclass + total_time_fought.seconds._dif + avg_TOTAL_STR_att_dif + avg_DISTANCE_att_dif + avg_TOTAL_STR_landed_dif + avg_CTRL_time.seconds._dif + avg_opp_BODY_landed_dif + win_by_Decision_Split_dif + avg_opp_BODY_att_dif + avg_DISTANCE_landed_dif + weightclass + avg_SIG_STR_landed_dif + avg_BODY_landed_dif + weightclass + avg_HEAD_landed_dif + avg_LEG_landed_dif + avg_SIG_STR_att_dif + avg_BODY_att_dif + avg_SUB_ATT_dif + avg_HEAD_att_dif + avg_LEG_att_dif + avg_opp_TD_pct_dif + avg_opp_CLINCH_att_dif + avg_opp_KD_dif + weightclass + winperc_dif + avg_opp_SIG_STR_pct_dif + avg_opp_TD_landed_dif + longest_win_streak_dif, data = train, family="binomial")
```
...
lmfit2 <- glm(win ~ age_dif + reach_dif + avg_opp_CTRL_time.seconds._dif + weightclass + avg_TD_att_dif + win_by_Decision_Split_dif , data = train, family="binomial")
...
```{r}
pred <- predict(lmfit2, test, type = "response")
    ypred <- ifelse(pred > 0.5, 1, 0)
    accuracy <- length(ypred[test$win==ypred]) / length(ypred)
    #print(sprintf("predictors: %s", predictors))
    print(sprintf("Test prediction accuracy: %f", accuracy))
